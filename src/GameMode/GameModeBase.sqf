...
CLASS("GameModeBase", "MessageReceiverEx")
	...
	METHOD(_setupMessageLoops)
		...
		if (!IS_NULL_OBJECT(T_GETV("messageLoopUnscheduled"))) then {
			CALLM(gMessageLoopUnscheduled, "addProcessCategoryUnscheduled", ["AIGroup" ARG 1 ARG 0 ARG 1]); // Interval, minObjPerFrame, maxObjPerFrame
	...
	public virtual METHOD(playerSpawn)
		...
		_newUnit addAction [format ["<img size='1.5' image='\A3\ui_f\data\GUI\Rsc\RscDisplayMain\infodlcsowned_ca.paa' />  %1", localize "STR_GMB_ATTACH_TO_GARRISON"], // title // pic: arrow pointing down
						...
						false, //hideOnUse
						"", //shortcut
						"call vin_fnc_attachUnitCond", //condition
						2, //radius
						false, //unconscious
						"", //selection
						""]; //memoryPoint
		...
		_newUnit addAction [format ["<img size='1.5' image='\A3\ui_f\data\GUI\Rsc\RscDisplayMain\infodlcsowned_ca.paa' /><img size='1.5' image='\A3\ui_f\data\GUI\Rsc\RscDisplayMain\menu_singleplayer_ca.paa' />  %1", localize "STR_GMB_TAKE_UNIT"], // title // pic: arrow pointing down and single man
						...
						false, //hideOnUse
						"", //shortcut
						"call vin_fnc_groupUnitCond", //condition
						5, //radius
						false, //unconscious
						"", //selection
						""]; //memoryPoint
		...
		_newUnit addAction [format ["<img size='1.5' image='\A3\ui_f\data\igui\rscingameui\rscunitinfoairrtdfull\ico_cpt_sound_off_ca.paa' />  %1", localize "STR_GMB_DISABLE_ALARM"], // title
						...
						false, //hideOnUse
						"", //shortcut
						"call vin_fnc_canDisableAlarm", //condition
						2, //radius
						false, //unconscious
						"", //selection
						""]; //memoryPoint
